...
$comp_title    = htmlspecialchars($settings['comp_title']    ?? 'تسجيل المسابقة',           ENT_QUOTES, 'UTF-8');
$comp_subtitle = htmlspecialchars($settings['comp_subtitle'] ?? 'سجّل بياناتك وكن جزءاً من المسابقة', ENT_QUOTES, 'UTF-8');
$comp_closed   = ($settings['comp_active'] ?? '1') === '0';
...
<head>
  ...
  <title>مسابقة مخازن العناية - شارك واربح</title>
  ...
  <style>
    ...
    .hero-bg-grid { position: absolute; inset: 0; display: grid; grid-template-columns: 1fr 1fr 1fr; }
    .hero-bg-grid img { width: 100%; height: 100%; object-fit: cover; filter: brightness(0.32) saturate(0.7); display: block; }
    .hero-overlay {
    ...
    .hero-banner .trophy { font-size: 58px; line-height: 1; margin-bottom: 18px; display: block; color: #FFCF06; filter: drop-shadow(0 0 20px rgba(255,207,6,0.5)); }
    .hero-banner h1 { font-size: clamp(22px,5vw,36px); font-weight: 800; color: #FFCF06; margin-bottom: 10px; font-family: 'Tajawal',sans-serif; }
    ...
    .form-card { background: #1a1a1a; border: 1px solid rgba(255,207,6,0.25); border-radius: 20px; padding: 36px 32px; }
    .form-section-title { font-size: 13px; font-weight: 700; color: #FFCF06; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 20px; padding-bottom: 10px; border-bottom: 1px solid rgba(255,207,6,0.25); font-family: 'Tajawal',sans-serif; }

    .field-group { margin-bottom: 20px; }
    .field-group > label { display: block; font-size: 14px; font-weight: 700; color: #f0f0f0; margin-bottom: 7px; font-family: 'Tajawal',sans-serif; }
    .field-group label span.req { color: #FFCF06; margin-right: 2px; }
    .input-wrap { position: relative; }
    .input-wrap > i { position: absolute; right: 14px; top: 50%; transform: translateY(-50%); color: #888; font-size: 15px; pointer-events: none; }
    .field-input { width: 100%; background: #0e0e0e; border: 1.5px solid rgba(255,255,255,0.1); border-radius: 10px; padding: 13px 42px 13px 14px; font-size: 15px; font-family: 'Tajawal',sans-serif; color: #f0f0f0; outline: none; transition: border-color .25s ease, box-shadow .25s ease; appearance: none; -webkit-appearance: none; }
    ...
    .gender-label { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 13px; border: 1.5px solid rgba(255,255,255,0.1); border-radius: 10px; background: #0e0e0e; cursor: pointer; font-size: 15px; font-weight: 700; color: #888; transition: all .25s ease; user-select: none; font-family: 'Tajawal',sans-serif; }
    .gender-option input:checked + .gender-label { border-color: #FFCF06; color: #FFCF06; background: rgba(255,207,6,0.08); box-shadow: 0 0 0 3px rgba(255,207,6,0.12); }
    .gender-label:hover { border-color: rgba(255,207,6,0.4); }

    .terms-wrap { display: flex; align-items: flex-start; gap: 12px; background: rgba(255,207,6,0.04); border: 1px solid rgba(255,207,6,0.25); border-radius: 10px; padding: 14px; cursor: pointer; }
    .terms-wrap input[type="checkbox"] { width: 20px; height: 20px; min-width: 20px; accent-color: #FFCF06; cursor: pointer; margin-top: 1px; }
    ...
    .btn-submit { width: 100%; margin-top: 28px; padding: 16px; background: #FFCF06; color: #000; border: none; border-radius: 12px; font-family: 'Tajawal',sans-serif; font-size: 18px; font-weight: 800; cursor: pointer; transition: opacity .25s ease,transform .25s ease; display: flex; align-items: center; justify-content: center; gap: 10px; }
    .btn-submit:hover  { opacity: .9; transform: translateY(-1px); }
    .btn-submit:active { transform: scale(.98); }
    .btn-submit:disabled { opacity: .5; cursor: not-allowed; transform: none; }
    .btn-submit .spinner { width: 20px; height: 20px; border: 2.5px solid rgba(0,0,0,0.2); border-top-color: #000; border-radius: 50%; animation: spin .7s linear infinite; display: none; }
    .btn-submit.loading .spinner  { display: block; }
    .btn-submit.loading .btn-text { display: none; }
    @keyframes spin { to { transform: rotate(360deg); } }

    .closed-banner { background: rgba(255,92,92,0.08); border: 1px solid rgba(255,92,92,0.3); border-radius: 12px; padding: 24px; text-align: center; color: #ff5c5c; font-size: 16px; font-weight: 700; font-family: 'Tajawal',sans-serif; }
    ...
    .swal-ref-card { background: rgba(255,207,6,0.06); border: 1px solid rgba(255,207,6,0.25); border-radius: 12px; padding: 16px 20px; text-align: right; }
    .swal-ref-card .ref-row { display: flex; align-items: center; gap: 10px; padding: 6px 0; font-size: 14px; color: #ddd; border-bottom: 1px solid rgba(255,255,255,0.05); font-family: 'Tajawal',sans-serif; }
...
<?php if ($body_code): ?>
<div style="position:absolute;width:0;height:0;overflow:hidden"><?php echo $body_code; ?></div>
<?php endif; ?>
...
<div class="hero-banner" id="hero-banner">
  ...
  <div class="hero-content">
    ...
    <h1><?php echo $comp_title; ?></h1>
    <p><?php echo $comp_subtitle; ?></p>
  </div>
</div>
    ...
    <div class="field-group">
      ...
      <div class="input-wrap">
        ...
        <input type="text" id="full_name" name="full_name" class="field-input"
               placeholder="محمد عبدالله السعيد" autocomplete="name" maxlength="100" />
      </div>
      ...
      <div class="field-group">
        ...
        <div class="input-wrap">
          ...
          <input type="tel" id="phone" name="phone" class="field-input"
                 placeholder="05XXXXXXXX" maxlength="10" dir="ltr"
    ...
    <div class="row-2">
      <div class="field-group">
        ...
        <div class="input-wrap">
          ...
          <input type="text" id="national_id" name="national_id" class="field-input"
                 placeholder="1XXXXXXXXX" maxlength="10" dir="ltr"
                 style="text-align:right" inputmode="numeric" />
        </div>
...
const rules = {
  ...
  gender:      () => { const val = document.querySelector('input[name="gender"]:checked')?.value; return val ? true : 'يرجى اختيار الجنس'; },
...
function showError(field, msg) {
  ...
  if (!el) return;
  if (msg) { el.textContent = msg; el.classList.add('show'); if (inp) inp.classList.add('error'); }
...
document.getElementById('national_id').addEventListener('input', function() { this.value = this.value.replace(/\D/g,'').slice(0,10); });
...
function showSuccess(data) {
  ...
  Swal.fire({
    ...
    didOpen: () => {
      ...
      if (p) { p.style.fontFamily="'Tajawal',sans-serif"; p.style.direction='rtl'; p.style.borderRadius='20px'; p.style.border='1px solid rgba(255,207,6,0.25)'; p.style.maxWidth='480px'; }
    },
  });
}
...
document.getElementById('reg-form').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    if (data.success) {
      ...
      Swal.fire({ icon:'error', title:'تنبيه', text: data.message || 'حدث خطأ، يرجى المحاولة مجدداً', confirmButtonText:'حسناً', confirmButtonColor:'#FFCF06', background:'#1a1a1a', color:'#f0f0f0', customClass:{popup:'swal2-popup'}, didOpen:()=>{ const p=document.querySelector('.swal2-popup'); if(p){p.style.fontFamily="'Tajawal',sans-serif"; p.style.direction='rtl';} } });
    }
  } catch {
    Swal.fire({ icon:'error', title:'خطأ في الاتصال', text:'تعذّر الاتصال بالخادم، يرجى المحاولة لاحقاً', confirmButtonText:'حسناً', confirmButtonColor:'#FFCF06', background:'#1a1a1a', color:'#f0f0f0' });
  } finally {
